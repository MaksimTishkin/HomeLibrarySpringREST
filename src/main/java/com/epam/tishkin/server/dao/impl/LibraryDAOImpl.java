...
import com.epam.tishkin.server.config.HibernateUtil;
...
public class LibraryDAOImpl implements LibraryDAO {
    ...
    public User userAuthorization(String login, String password) {
        try (Session session = HibernateUtil.getSession()) {
            User libraryUser = getUserByLogin(login);
            if (libraryUser != null) {
    ...
    public boolean addUser(User libraryUser) {
        try (Session session = HibernateUtil.getSession()) {
            Transaction transaction = session.beginTransaction();
            User visitor = session.get(User.class, libraryUser.getLogin());
            if (visitor != null) {
                return false;
            }
    ...
    public boolean addBookmark(Bookmark newBookmark, String login) {
        try (Session session = HibernateUtil.getSession()) {
            Transaction transaction = session.beginTransaction();
            User libraryUser = session.get(User.class, login);
            ...
            newBookmark.setUser(libraryUser);
